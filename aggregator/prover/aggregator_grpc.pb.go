...
func (c *aggregatorServiceClient) Channel(ctx context.Context, opts ...grpc.CallOption) (AggregatorService_ChannelClient, error) {
	...
	stream, err := c.cc.NewStream(ctx, &AggregatorService_ServiceDesc.Streams[0], AggregatorService_Channel_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &aggregatorServiceChannelClient{ClientStream: stream}
	return x, nil
}
...
func _AggregatorService_Channel_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AggregatorServiceServer).Channel(&aggregatorServiceChannelServer{ServerStream: stream})
}
